...
description: 通过演练了解“We.Finance”车险续订参考网站。
...
`We.Finance`网站是一个金融服务网站，旨在帮助您学习AEM Forms的交互式通信功能。

阅读有关`We.Finance`车险使用案例的详细演练，该用例展示了AEM如何形成及其与Microsoft® Dynamics的集成有助于个性化金融服务公司的客户体验。 交互式演练旨在简化在金融公司中复杂数字交易和客户通信的实施。

**历程以用例开始：**

Sarah Rose是`We.Finance`的现有客户，并且已购买汽车保险单。 现在正是莎拉续保的时候。 格洛丽亚·里奥斯是她的保险代理人。 `We.Finance`网站通过电子邮件向Sarah发送有关其策略续订的提醒。 Sarah按照电子邮件中的说明成功完成了此过程。

## 自动保险申请演练 {#auto-insurance-application-walkthrough}

`We.Finance`自动保险申请方案是用户的视觉叙述，基于两个角色：

* Sarah Rose，`We.Finance`客户
* Gloria Rios，保险代理，`We.Finance`

### Gloria从`We.Finance`发送保险单续订通信 {#gloria-sends-an-insurance-policy-renewal-communication-from-we-finance}

Gloria登录到AEM实例，单击&#x200B;**自动保险续订，**，然后单击&#x200B;**打开代理UI**。 单击会使用Sarah Rose的保单详细信息预填充保险文档。 Gloria单击&#x200B;**提交**，屏幕上将显示消息“已启动提交”，然后在几秒钟内显示“已成功提交”。
...
### Sarah收到来自`We.Finance`的保单续订通信并决定续订 {#sarah-receives-an-insurance-policy-renewal-communication-from-we-finance-and-decides-to-renew}

Sarah收到一封包含来自`We.Finance`的附件的电子邮件，提醒Sarah她的汽车保险单即将过期。 附件是Sarah汽车保险信的打印版。

Sarah单击选项&#x200B;**立即续订**&#x200B;并定向到其汽车保险信的Web版本。 在此信的顶部，Sarah查找策略在过期之前剩余的时间。 本页提供了保单概览。 它详细说明了保单编号、应付金额、折扣优惠和忠诚度奖励。 单击策略底部的&#x200B;**立即续订**。

![ref1](assets/ref1.png)

#### 工作原理 {#how-it-works}

使用交互式通信的多渠道功能创建汽车保险书的Web和打印输出。
...
`We.Finance`网站会将Sarah带到“付款”页面。 Sarah用她的记录重新检查她的保险单号码和到期日期。 在页面的右侧，Sarah检查续订的付款摘要，总金额享受10%的溢价折扣。
...
当Sarah填写信用卡详细信息并单击提交时，将处理她的信用卡付款，并且屏幕上将显示一条在自适应表单中配置的感谢消息。

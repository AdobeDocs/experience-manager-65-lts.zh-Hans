...
| 权限 | 您可以为用户或组分配权限。 可控制以下内容：<ul><li>与特定页面/节点相关的权限。 请参阅[设置权限](#setting-permissions)。 </li><li>与创建和删除页面以及层次结构修改相关的权限。 ??? 允许您[分配权限](#settingprivileges)，例如层次结构修改，允许您创建和删除页面，</li><li>与[复制特权](#setting-replication-privileges)相关的权限（通常是从创作环境到发布环境），依据路径进行分配。</li></ul> |

...
>[!IMPORTANT]
>
>CRX Explorer (`/crx/explorer/index.jsp`)在AEM 6.5 LTS环境中不可用，不能用于创建服务用户。 通过CRX Explorer创建的现有服务用户继续正常运行。 对于新的服务用户，请使用下面描述的方法之一。

>[!NOTE]
>
>在JCR节点级别，没有与服务用户关联的mixin类型。 这意味着系统用户节点没有直接附加的访问控制策略。 访问控制是单独管理的，例如，通过RepoInit ACL语句或存储库级别的ACL配置。

### 使用Sling存储库初始化(RepoInit) {#creating-service-user-repoinit}

推荐的方法是使用[Sling存储库初始化(RepoInit)](https://sling.apache.org/documentation/bundles/repository-initialization.html)创建服务用户。 RepoInit允许您使用简单的脚本语言以声明方式定义服务用户及其ACL。

要使用RepoInit创建服务用户，请将`scripts`属性添加到`org.apache.sling.jcr.repoinit.RepositoryInitializer`的OSGi配置中：
...
`with path system/cq`指令将服务用户放置到存储库中的`/home/users/system/cq`下。 您可以选择与项目的组织结构匹配的路径（例如，`system/myproject`）。 如果中间路径节点不存在，请使用`with forced path`自动创建它们。

建议使用此方法，因为它：

* 将服务用户和权限定义为代码，使它们受版本控制且可重现
* 在存储库初始化期间自动处理创建
* 可在AEM 6.5 LTS和AEM as a Cloud Service环境中使用，尽管Sling版本之间可能存在细微的语法差异 — 请参阅Target平台的RepoInit文档

### 使用内容包 {#creating-service-user-content-package}

您还可以通过在内容包中包含`.content.xml`来创建服务用户。 确保您已设置`rep:authorizableId`，并且主要类型为`rep:SystemUser`。 内容包安装期间需要有效的`jcr:uuid`属性才能正确创建用户。 您可以使用标准UUID v4生成器（例如，`uuidgen`命令行工具或任何在线UUID生成器）生成UUID。 `.content.xml`应如下所示：

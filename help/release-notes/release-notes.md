...
description: 查找Adobe Experience Manager 6.5 LTS Service Pack 2的最新发行信息。
...
* 添加了运行时应用程序编程接口(API)，以在AEM Forms中切换旧版可扩展标记语言(XML)导出模式，并替换了`Dcom.adobe.fd.forms.export.legacy`参数。 此增强使用户能够更有效地切换导出模式，从而提高工作流的灵活性。 (FORMS-23115)
...
## 修复了6.5 LTS Service Pack 2中的问题 {#fixed-issues}
...
* 现在，“链接选择”对话框会将编程标签与上一级树项目的屏幕标签对齐。 箭头键导航会触发每个项目的可靠屏幕阅读器公告，并删除误导性的标签输出。 （SITES-24710）
...
AEM Headless需要一个专用发布分支来避免依赖关系，以及避免与主线内部版本的捆绑包版本冲突。 该更新添加了`release/6.5lts`个Headless分支，并使依赖项集和捆绑包版本保持一致。 Jenkins现在可以干净地构建Headless代码库，而不会出现版本冲突。 (SITES-36585)
...
* 在开发和暂存环境中，“通用编辑器打开”命令现在默认为预览模式。 该命令添加了`preview=true`，这样可保持作者检查与预览上下文一致，并避免意外打开生产环境。 (SITES-33839)
...
* 用户遇到表单数据模型(FDM)编辑器`Data Source / Enter Keyword`功能问题。 此问题会影响搜索和选择数据源的能力。 (FORMS-23971)
* 在移动设备上，自适应Forms中的表组件在顶部呈现了一个隐藏的标题，导致屏幕阅读器错误地声明了内容。 这会影响依赖屏幕阅读器进行导航的用户。 (FORMS-23754)
* 用户遇到基于核心组件的自适应Forms引用标记为granite:InternalArea的资源类型的问题，这影响了本地Forms加载项中多个granite组件的功能。 (FORMS-23632)
* 升级到AEM 6.5 LTS SP1后，表单提交失败。 用户遇到缺少com.adobe.cq.social.commons.CollabUtil的情况，这导致JSP编译错误和电子邮件操作失败。 (FORMS-23457)
* 用户在基于Foundation Components的自适应Forms中遇到验证码无法正确翻译的问题。 这会影响非英语用户准确完成表单的能力。 (FORMS-23426)
* 用户遇到表单提交失败，出现SAXParseException：“prolog中不允许有内容”(HTTP 500)。 出现此问题的原因是，预填充数据XML中存在空值，导致服务器端XML解析失败。 (FORMS-22633)
* 用户体验了自适应Forms未通过Web内容无障碍准则(WCAG)审核。 原因是表单的选项卡导航标记无效。 也就是说，非列表元素将呈现为列表的直接子元素，其中仅允许列表项。 此问题使表单无法通过必须满足法律或内部合规性要求的辅助功能验证器和受影响的组织。 (FORMS-22101)
* 用户遇到记录文档(DoR) /提交PDF中的辅助功能问题，其中空白表单字段未标记为表单元素。 这给屏幕阅读器带来了困难，影响了残障用户有效导航和完成表单的能力。 (FORMS-21989)
* 用户遇到了在表单加载期间子面板中组件的脚注未显示的问题。 当带有脚注的项目是页面上的最后一个组件时，会发生此问题。 (FORMS-21925)
* 用户在AEM Forms编辑器中选择组件时遇到问题。 当在选项卡之间导航并返回第一个选项卡时，某些容器变为不可选，从而阻止轻松识别和交互。 (FORMS-21814)
* 用户在“自适应Forms”功能板中遇到安全漏洞。 具体而言，在startpointcontrol.js文件中发现了一个跨站点脚本(XSS)问题，该问题可能会允许执行恶意脚本。 (FORMS-20679)
* 在JBoss® EAP 8上的AEM Forms 6.5 LTS群集部署中，`domain/configuration/domain_oracle.xml`、`domain_mysql.xml`和`domain_mssql.xml`文件不再包含重复的`<security>`标记，这些标记导致无效的XML并阻止域控制器启动。 (FORMS-24687)
* 在Turnkey模式下，现在可在全新安装和升级期间正确应用数据库端口更新。 在全新安装模式下，用户可以从所有可用端口中进行选择，而在升级模式下，在升级过程中会正确引用lc_turnkey.xml中更新的数据库端口。 (FORMS-24689)
* 在Linux®上设置JBoss® EAP 8.0时，在Windows上修改的Shell脚本不再因CRLF行结尾而导致`/bin/sh^M: bad interpreter or $'\r': command not found`错误。 (FORMS-24688)
...
* AEM现在可避免在生成的`WEB-INF/resources/provisioning/model.txt`中为`contributor`组输入重复的ACL条目。 WAR输出现在包含一个一致的ACL块，以防止在查看期间混淆权限差异。 (GRANITE-63269)
* 在捆绑刷新操作期间，AEM不再清除反序列化防火墙和设置。 更新了筛选器注册逻辑，使活动防火墙实例与保存的配置保持一致，因此无需重新启动即可保持启用保护状态。 (GRANITE-61382)
* Felix Web控制台在访问`/system/console`期间不再引发间歇性`NullPointerException`错误。 更新了ServiceTracker处理，防止出现null跟踪器状态。 在重复请求和自动验证期间，控制台登录和导航保持稳定。 (GRANITE-61042)
  ...
  **您应该做什么**
   ...
   1. 将它们替换为相应的新PID：
      ...
      * 全局PID： `org.apache.sling.jcr.base.LoginAdminAllowList`
      ...
      有关其他详细信息，请参阅[管理登录的已弃用包](https://sling.apache.org/documentation/the-sling-engine/service-authentication.html#deprecated-approach-to-allowlist-bundles-for-administrative-login)。
...
适用于AEM 6.5 LTS SP2的UberJar使用AEM 6.5 LTS UberJar版本6.6.2。 您可以从Maven中央存储库检索相应的UberJar工件。 与AEM 6.5不同，AEM 6.5 LTS将公共API和已弃用的API分隔成两个不同的工件。
...
建议客户检查其当前部署中是否使用了此类特性/功能。 制定计划以更改其实施，从而使用提供的替代方案。
...
| Sites | [SPA 编辑器](/help/sites-developing/spa-overview.md) | 在AEM中管理Headless内容的首选编辑器是：<br>- [用于可视化编辑的通用编辑器](/help/sites-developing/universal-editor/introduction.md)。<br>- [用于基于表单的编辑的内容片段编辑器](/help/assets/content-fragments/content-fragments-managing.md)。 | 6.5 LTS GA |
...
* 在Configuration Manager中，当未选择模块或仅选择有限的组件时，在AEM Forms 6.5 LTS JEE全包自定义模式下Bootstrap数据库初始化会失败。 失败是由于缺少依赖项(xalan-2.7.2.jar)，从而导致错误。 将JAR文件添加到adobe-livecycle-jboss.ear\lib解决了此问题。 (FORMS-24690)
* 在JBoss® EAP 8上运行的Forms JEE LTS部署中，Reader扩展UI可能会失败，并显示内部服务器错误。 (FORMS-24894)
* 在JBoss®上运行的Forms JEE LTS上，与电子邮件相关的功能可能会失败。 尝试使用电子邮件功能时，服务器可能会记录类似于`Error IMAPProvider not a subtype`的错误。 (FORMS-24892)
* 在Linux®平台上，Forms JEE LTS要求在运行Configuration Manager之前正确设置`LFS_Foundation.properties`中的`OSFileSetIntendedFor`属性。 如果未更新，则配置可能无法针对Linux®进行适当定制，这可能会导致运行时或部署问题。 若要解决此问题，请在运行安装程序之后，执行Configuration Manager之前，导航到`configurationManager/config/solcomp/`，打开`LFS_Foundation.properties`，设置`OSFileSetIntendedFor=Linux`，保存文件，然后运行Configuration Manager。 (FORMS-24741)
...
要解决此问题，请从[软件分发](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.2-hotfix-GRANITE-65388-1.0.zip)安装修补程序。 由于该修补程序包含一个低级`oak-segment-tar`捆绑包，因此实例会在安装后重新启动。

规划应用实例时的停机时间。 对于脱机压缩，请使用相应的[`oak-run` jar](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.88.1-B006.jar)，该变量也在Software Distribution中提供。
...
以下文本文档列出了此[!DNL Experience Manager] 6.5 LTS Service Pack 2版本中包含的OSGi包和内容包： <!-- UPDATE FOR EACH NEW RELEASE -->

* [Experience Manager 6.5 LTS Service Pack 2](/help/release-notes/assets/65lts_sp2_bundles.txt)中包含的OSGi包列表<!-- UPDATE FOR EACH NEW RELEASE -->
* [Experience Manager 6.5 LTS Service Pack 2](/help/release-notes/assets/65lts_sp2_packages.txt)中包含的内容包列表<!-- UPDATE FOR EACH NEW RELEASE -->

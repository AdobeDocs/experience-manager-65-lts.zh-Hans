...
AEM 6.5 LTS SP2现在包括用于[内容片段和模型管理](https://developer.adobe.com/experience-cloud/experience-manager-apis/api/stable/sites/65lts/)和[启动](https://developer.adobe.com/experience-cloud/experience-manager-apis/api/stable/sites/launches/)的OpenAPI。 这些API提供对内容片段的访问以及用于创作和计划的启动。 它们使用与AEM即云服务相同的现代OpenAPI。
...
* 作者在编辑过程中将鼠标指针悬停在组件浏览器中的项目上时，文本组件会失去键盘焦点。 这会中断键入并触发WCAG 3.2.1下的辅助功能故障。此修复方法可防止悬停样式在组件浏览器交互期间转移焦点，并使文本组件保持聚焦。 （SITES-35370）
* 更正了描述富文本字段中的焦点管理，该字段使用Tab键阻止了向前导航。 用户在RTE中陷入卡顿，因为该组件依赖非标准键盘命令来转移焦点，从而打破了预期的对话框导航。 此更改强制执行标准的键盘交互操作，并保留整个对话框中的逻辑Tab键顺序。 （SITES-35228）
* 修复了Sites编辑器中中断页面创作期间预期行为并导致组件交互不一致的问题。 作者遇到的UI响应不可靠，干扰了标准编辑任务并降低了工作流程效率。 此更新优化了底层编辑器逻辑，并恢复了跨受影响组件的稳定、可预测的交互。 （SITES-35227）
* 导致页面编辑器中的资源选择器崩溃，无法在特定的页面编辑场景中加载。 现在，作者在编辑页面时选择或浏览资源时，可以正常打开和使用资源选择器。 此更改恢复对加载失败中断的资源选择工作流的一致访问。 （SITES-35226）
* 消除了站点编辑器中的问题，该问题在页面交互期间导致不一致行为并中断了标准创作工作流。 该缺陷导致意外的UI响应，干扰了组件配置和内容更新。 此更新可稳定受影响的功能，并恢复跨页可靠地执行编辑操作。 （SITES-35225）
* 消除了站点创作界面中导致页面编辑期间行为不一致并中断正常工作流程的缺陷。 作者遇到意外的UI响应，这些响应会干扰组件交互和内容更新。 此更新使受影响的功能更加稳定，并在所有编辑场景中恢复可靠、可预测的行为。 （SITES-35224）
* AEM Sites现在包括对图像的`alt`文本支持，以满足ADA和WCAG要求。 页面输出不再忽略`alt`属性，因此屏幕阅读器会收到正确的替换文本。 （SITES-27153）
* 修复了`Note Add`工具栏布局，以便“添加”按钮不再与320px视区宽度处的标题重叠。 改进了小屏幕Reflow，使控件在400%缩放期间仍可读且可用。 （SITES-25376）
* 修复了链接选择对话框错误导致屏幕阅读器公告缺失的问题。 UI现在通过状态消息容器发布错误文本，因此NVDA会在消息出现时立即读取它。 （SITES-25368）
* 从侧边栏资源列表中删除了ARIA网格和网格单元格角色。 恢复了标准列表语义和键盘焦点顺序，这改进了屏幕阅读器导航并减少了额外的制表位。 （SITES-25361）
* 更正了侧边栏Assets中的焦点顺序。 键盘用户现在可以通过一致的选项卡路径访问每个资产操作，包括“编辑”。 （SITES-25360）
* 修复了Search Assets模式中视区宽度为320px的布局溢出问题。 模态内容现在会重新流动并保持可读状态，因此控件不再重叠或溢出对话框。 （SITES-25330）
* 更正了“编辑”按钮的NVDA输出。 现在，NVDA将宣布“编辑”操作，而不是“预览按钮已按下”。 （SITES-25320）
* 修正了未命名的人口统计工具栏文本输入导致无提示或一般屏幕阅读器输出的问题。 每个输入现在都会显示一个清晰的基于标签的可访问名称，这改进了键盘和辅助技术导航。 （SITES-25316）
* 更正了布局预览导航期间人口统计工具栏的键盘焦点顺序。 选项卡导航现在可直接从人口统计按钮移至工具栏控件，而无需跳到辅助工具栏。 （SITES-25305）
...
* 修复了“删除注释”对话框的键盘焦点处理问题。 现在，“取消”将焦点返回到打开该对话框的`Delete`控件，而不是“确认十六进制值”控件。 取消后，屏幕阅读器不会再公告不相关的对话框内容。 （SITES-25258）
* 修正了“批注模式”对话框的焦点处理问题。 现在打开对话框会将焦点放在对话框标题上，并停止NVDA读取画布内容和不相关的对话框文本。 键盘导航现在会保留在对话框中，直到关闭。 （SITES-25257）
...
* 激活批注现在会将键盘焦点移至批注内容，而不是“退出批注”按钮。 Tab键顺序遵循逻辑顺序，使相关控件可访问而无需反向导航。 （SITES-25241）
* 在键盘导航过程中，“设置日期”和“退出时间扭曲”链接缺少可见的焦点指示器。 UI现在呈现一种独特的高对比度焦点样式，因此用户可以快速识别活动链接。 （SITES-25232）
* “预告模式”标题不再阻止键盘用户移动对话框。 现在，键盘控件支持拾取、移动和拖放操作，这提高了屏幕阅读器的可用性和整体可操作性。 （SITES-25226）
...
* 列表视图表头现在可通过ARIA显示排序状态。 执行列排序操作后，屏幕阅读器将宣布升序或降序。 （SITES-24943）
...
* 现在，每当用户将工具提示图标悬停时，`Delete Modal`工具提示都会显示。 焦点操作现在显示相同的工具提示文本，这改进了鼠标和键盘用户的重复访问。 （SITES-24778）
* 现在，左边栏导航在用户配置边栏后，遵循预期的键盘焦点顺序。 选项卡焦点位于选定的左边栏区域，而不是“切换显示”，这提高了屏幕阅读器导航的清晰度。 （SITES-24754）
* 修复了“用户首选项”模式中色板导航期间的NVDA反馈不正确的问题。 NVDA现在会读取接收焦点的样本的标签，这样会删除误导性的颜色输出。 样本集现在支持一致的键盘导航和清晰的选择感知。 （SITES-24739）
* 减少了`Spin`控件的详细NVDA输出。 删除了重复输入标签的冗余组标签，因此NVDA会公告一次控制名称。 现在，键盘和屏幕阅读器导航功能可提供单一、清晰的公告。 （SITES-24725）
* “轮播”对话框现在将焦点放在对话框标题而不是“项目”选项卡上。 取消和Esc会将焦点恢复到启动对话框的控件，从而减少详细的NVDA输出。 （SITES-24716）
* 现在，链接选择对话框将程序化标签与最后一层树项目的屏幕标签对齐。 箭头键导航会触发每个项目的可靠屏幕阅读器公告，并删除误导性标签输出。 （SITES-24710）
* 现在，链接打开选择对话框在320像素视区下正确重排。 内容不再超出模式或截断，并且模式不再显示水平滚动条。 （SITES-24709）
...
* 侧栏资源面板现在包括一个闭合控件。 “关闭”将键盘焦点返回到边栏切换开关，并防止通过面板内容强制使用Tab键。 （SITES-24489）
* 键盘Tab键现在可以访问管理表中的按钮和链接。 用户不再依赖箭头键单元格导航来查找交互式控件。 （SITES-24285）
...
站点控制台列表视图设置未反映列表视图中显示的列。 该对话框打开时未清除复选框，且选中的列计数不正确。 修复将对话框状态与活动网格列同步，并更新计数器以匹配实际列可见性。 （SITES-38576）
...
* 内容片段编辑器现在可以本地化“标签”标签。 编辑器还会本地化“收藏夹”标签，因此UI文本与所选区域设置匹配。 （SITES-977）
...
* 在最近UI样式更改后，内容片段RTE显示布局和视觉问题。 Service Pack 2改进了RTE样式，因此工具栏和可编辑区域可以正确呈现并保持可读性。 内容片段编辑器现在与页面编辑器外观和行为一致。 （SITES-38684）
* 从Polaris资产选择器中删除IMS范围会中断与投放端点的内容片段集成。 作者在打开远程资产选择器和选择资产时遇到了失败。 该更新重新添加所需的IMS范围并恢复稳定的传送层访问。 （SITES-35837）
* “关联的内容”面板不再呈现硬编码的“未定义”占位符。 内容片段编辑器现在通过本地化资源解析该文本，因此编辑器可以看到翻译的UI文本。 （SITES-33675）
...
* 内容片段编辑器现在会显示允许的资源类型的已翻译名称。 作者配置内容引用限制时，选取器列表不再混合内部字符串和仅英文标签。 （SITES-29699）

#### [!DNL Content Fragments] - GraphQL API {#sites-graphql-api-65-lts-sp2}

* 改进了GraphQL查询验证处理，以停止由筛选器执行错误导致的部署失败。 该缺陷在应用程序启动期间生成异常，并在受影响的环境中阻止成功转出。 该修订版可确保一致的验证行为，并能够实现顺利部署，而无需运行时查询验证中断。 (SITES-34301)关键
...
* 重构将功能绑定到禁用的切换时，嵌套的内容片段模型停止工作。 修复程序恢复嵌套模型支持，而无需切换更改。 作者可以在模型编辑器中再次创建和使用嵌套模型。 (SITES-38681)关键

* “内容片段模型”过滤器面板不再显示未本地化的字符串。 AEM现在显示所有区域设置的本地化过滤器标签和本地化状态值。 （SITES-30863）
* 内容片段模型编辑器现在可呈现锁定警告对话框的本地化字符串。 UI会将非本地化英语消息替换为跨支持语言的区域设置资源。 （SITES-28592）

#### [!DNL Content Fragments] - REST API{#sites-restapi-65-lts-sp2}

AEM Headless需要一个专用发行分支以避免依赖项和包版本与主行内部版本冲突。 此更新添加了6.5lts版本的Headless分支，并使依赖项集和捆绑包版本保持一致。 Jenkins现在可以干净地构建Headless代码库，而不会出现版本冲突。 （SITES-36585）
...
* 更改站点创作体验，以解决破坏标准页面编辑工作流程的不一致行为。 作者在组件交互期间遇到意外结果，这会干扰内容更新并降低可靠性。 此更改可恢复稳定的编辑器行为，并确保在受影响的场景中一致地执行创作操作。 (SITES-35162)关键
...
* 启动项UI显示了内容片段启动项控件，即使产品未在触屏UI中公开内容片段启动项也是如此。 此更改从cq-launches-content删除内容片段启动代码路径，并调整启动列表过滤。 作者现在可以看到一致的页面启动选项，没有内容片段启动条目。 （SITES-35633）
...
* AEM Launch现在为6.5 LTS分支运行专用的Jenkins管道。 管道每晚运行，并通过电子邮件发送故障警报。 该设置增加了测试覆盖率并提前捕获回归。 （SITES-35293）
* AEM 6.5 LTS现在发布了更新的Launches API捆绑包，其中包含对齐的人工因素版本。 该捆绑包将跟踪主代码行，同时保持正确的6.5 LTS发行版本。 此更新稳定了6.5 LTS栈栈中的Launches API消耗。 （SITES-35292）
* AEM 6.5 LTS现在包含一个更新的launches-core捆绑包，其中包含对齐的依赖关系版本。 此更新会为片段UUID和参考UUID数据类型添加启动项核心处理。 现在，启动项处理可保持启动项和内容片段工作流之间的行为一致。 （SITES-35290）
* 改进了站点编辑器，以解决中断正常页面创作工作流的不一致行为。 作者遇到意外的组件交互，这会干扰内容更新并降低编辑可靠性。 此更改可恢复一致的UI状态管理，并确保在受影响的场景中可预见地执行创作操作。 （SITES-35138）
* 启动“编辑”现在显示本地化的错误文本，而不是硬编码的`Provided path is not a launch`字符串。 现在，当“编辑”收到无效的启动路径时，UI会呈现跨语言的翻译消息。 （SITES-33360）
* AEM 6.5 LTS现在包括启动OpenAPI侧端口工作。 此次更新将“启动API捆绑包”、“内容包”和所需的“快速启动”工件对等起来，并启用“内容片段”启动具有稳定CI验证的OpenAPI方案。 （SITES-32050）
* 启动UI现在会本地化“已覆盖”模板标签。 现在，模板覆盖详细信息会显示已翻译的文本，而不是仅英文字符串。 （SITES-29525）
* AEM解析了&#x200B;**站点** > **启动** > **编辑**&#x200B;中缺少的本地化密钥。 现在，用户会看到已翻译的错误消息，而不是原始的“无法更新启动源列表”字符串。 （SITES-21499）
* 启动升级UI现在会显示已本地化的状态标签和操作。 预览区域显示&#x200B;**已删除**、**新建**&#x200B;和&#x200B;**视图**&#x200B;的已翻译文本，而不是原始英语字符串。 （SITES-13540）
* 启动创建现在显示本地化的错误消息。 UI不再显示原始英语字符串，如`Unable to create launch page`、`Source root resource is not a page`或`Mandatory parameter is missing`。 （SITES-13085）
...
* 在内容更改期间，管理员对MSM推送修改处理的可见性有限。 修复添加了关于MSM事件接收和转出执行的详细日志记录。 现在，调试输出会显示触发的事件、更改的内容路径以及触发更改的人员。 （SITES-38029）
* AEM在“Blueprint推广日期”字段中修复了本地化布局问题。 日期提示现在适合控件，并且仍然可以在支持的语言（包括`fr_FR`）中打开。 （SITES-14961）

<!-- #### Page editor{#sites-pageeditor-65-lts-sp2} -->

#### 复制{#sites-replication-65-lts-sp2}

页面编辑器发布现在可处理包含选择器或后缀的URL。 现在，发布请求会发送JCR页面路径，而不是选择器或后缀URL字符串，因此激活已完成，内容开始生效。 现在，复制会在失败时返回错误状态，从而阻止出现错误的“发布已开始”消息。 （NPR-43288）
...
* 现在，通用编辑器打开命令在开发和舞台环境中默认为预览模式。 该命令添加preview=true，使作者检查与预览上下文保持一致，并避免意外打开“生产”。 （SITES-33839）

### [!DNL Assets]{#assets-65-lts-sp2}

Assets Relate现在适用于包含空格的文件名。 更新的“关系”客户端逻辑现在可以正确处理包含空格的路径，并避免在关系选择过程中出现`undefined`个源错误。 现在将打开“关系”对话框并保存关系，而不使用UI挂起或旋转器。 DAM用户无需重命名文件即可关联、派生和取消关联资源。 (Assets-56418)
...
* AEM现在可以避免在生成的`contributor`中出现`WEB-INF/resources/provisioning/model.txt`组的重复ACL项。 WAR输出现在包含一个一致的ACL块，以防止在查看期间混淆权限差异。 (GRANITE-63269)
* 在捆绑刷新操作期间，AEM不再清除反序列化防火墙和设置。 更新了筛选器注册逻辑，使活动防火墙实例与保存的配置保持一致，因此无需重新启动即可保持启用保护状态。 (GRANITE-61382)
...
升级Service Pack后打开JSP文件时，CRXDE Lite不再显示空白选项卡。 AEM现在附带匹配的CodeMirror核心代码和附加代码，可防止严重的浏览器错误并使编辑器保持可用。 (GRANITE-64333)

#### Granite{#foundation-granite-65-lts-sp2}

表达式安全验证器现在可处理空或null OSGi配置值。 它应用安全默认值，忽略空数组，并记录清晰的日志，从而防止NullPointerException和不可预知的验证结果。 （NPR-43163）
...
AEM 6.5 LTS Service Pack 2需要S3 Connector 1.60.10或更高版本。 S3数据存储配置现在包括`crossRegionAccess`和`mode`，因此管理员可以启用跨区域存储桶访问，并在需要时将存储切换到GCP。 `s3EndPoint`现在需要与`s3Region`对齐的区域，或者保持为空，因此驱动程序将生成终结点。 (GRANITE-64873)


#### 快速入门{#foundation-quickstart-65-lts-sp2}

* Sling更新管理登录允许列表以使用包容性术语和新配置PID。 此更改与Sling JCR Base 3.2.0一致。 (GRANITE-63756)

  **影响**

   * Sling已弃用这些PID，您应该从配置中删除它们：
...
这些旧配置使用属性，如`whitelist.name`和`whitelist.bundles`。
   ...
   * Sling仍为已弃用的PID提供部分向后兼容性，但不要将其用于新配置。 请改用较新的`LoginAdminAllowList.*`个PID。
   * 请勿同时运行已弃用的配置和新的允许列表配置。 混合配置可能会产生模糊性，并产生意外行为。 迁移到AEM 6.5 LTS SP2时，请完全删除已弃用的PID。
  ...
  **您应该做什么**
   ...
   1. 使用适当的新PID替换它们：
      ...
      * 全局PID: `org.apache.sling.jcr.base.LoginAdminAllowList`
      ...
      有关其他详细信息，请参阅[弃用的方法将包列入管理登录的允许列表](https://sling.apache.org/documentation/the-sling-engine/service-authentication.html#deprecated-approach-to-allowlist-bundles-for-administrative-login)。

* AEM 6.5 LTS SP2更新了用于Sling、Oak和Felix的基础层捆绑包。 这些升级增强了核心运行时稳定性，并调整了整个平台中的依赖关系版本。 (GRANITE-61874)
...
由于XLIFF格式问题，AEM翻译在Java 17或Java 21上不再失败。 导出管道现在可生成翻译提供商接受的符合标准的XLIFF。 此更改将删除翻译作业中断，并恢复AEM和翻译服务之间的可预测切换。 翻译工作流现在可以在支持的Java运行时保持稳定。 （CQ-4360217）

#### 工作流{#foundation-workflow-65-lts-sp2}

EmailNotificationService-Processor不再在工作流通知处理期间触发重复的“未找到区段”错误。 更新的异常处理检测到SegmentNotFoundException并停止处理循环，而不是继续无效读取。 在收件箱和工作项访问期间，工作流执行保持稳定，并记录噪音下降。 (GRANITE-62635)
...
适用于AEM 6.5 LTS SP2的UberJar使用AEM 6.5 LTS UberJar版本6.6.0。您可以从Maven中央存储库检索相应的UberJar工件。 与AEM 6.5不同，AEM 6.5 LTS将公共API和已弃用的API分隔成两个不同的工件。

要针对公共API进行编译，请使用以下内容：
...
另请参阅[更新AEM Uber Jar版本](/help/sites-deploying/upgrading-code-and-customizations.md#update-the-aem-uber-jar-version)。
...
1. 验证AEM 6.5 LTS实例是否正常并可访问。
1. 从软件分发下载Quickstart JAR（例如，`cq-quickstart-6.6.x.jar`）。
1. 停止正在运行的实例。
...
* 在开始之前，先进行完整的可恢复备份（存储库以及任何外部数据存储区）。
* 查看 Adobe 的就地升级指南及技术要求（LTS 建议使用 Java 17 或 21）。

>[!NOTE]
>
>上面所示的文件名（例如，`cq-quickstart-6.6.x.jar`）反映了此LTS发行版中观察到的Quickstart项目命名；请始终使用从软件分发中下载的确切文件名。
...
>[!NOTE]
>
> 如果要从旧的6.5 SP直接升级到LTS SP1，请按照6.5到6.5 LTS GA [升级](/help/sites-deploying/upgrade.md)的说明进行操作。
...
Adobe不断检讨和改进产品功能，以通过使旧功能现代化或替换旧功能而为客户提供更大的价值。 实施这些更改时，会仔细考虑向后兼容性。

为确保透明度和充分的规划，Adobe遵循了Adobe Experience Manager(AEM)的弃用程序：
...
| Sites | AEM Assets REST API中的内容片段支持 | AEM 6.5 LTS SP2为内容片段和模型管理提供了现代化的OpenAPI，因此现在不再支持AEM Assets REST API中的旧版内容片段支持端点。<br>Adobe打算在生命周期终止公告之前保留这些旧终结点可用。 Adobe不计划对已弃用的端点进行进一步的增强。 | 6.5 LTS SP2 |
...
### 脱机压缩后联机压缩期间存储库损坏(GRANITE-65146) {#repository-corruption-during-online-compaction-after-offline-compaction-granite-65146}

如果脱机压缩之前在JCR存储库上运行，则用户在联机压缩期间可能会遇到存储库损坏的情况。 在此情况下可能会发生`SegmentNotFoundException` (SNFE)，并且可能导致存储库损坏。

要解决此问题，请从[软件分发](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.2-hotfix-GRANITE-65388-1.0.zip)安装修补程序。 由于修补程序包含低级别`oak-segment-tar`捆绑包，因此实例将在安装后重新启动。

规划应用实例时的停机时间。 若要离线压缩，请使用相应的[oak-run jar](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.88.1-B006.jar)，该变量也在Software Distribution上提供。

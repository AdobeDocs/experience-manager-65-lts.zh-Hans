...
* 在配置期间为AEM管理员凭据添加了Configuration Manager (LCM) Express Mode提示，替换了内置的默认凭据。 此增强功能通过6.5 LTS SP2 JEE修补程序提供。 (FORMS-26704)
...
* 当作者在编辑过程中将鼠标悬停在组件浏览器中的项目上时，“文本”组件会失去键盘焦点。 这会中断输入，并触发 WCAG 3.2.1 的一个无障碍性错误。 此修复可防止悬停样式转移焦点，并在与组件浏览器交互期间使“文本”组件保持焦点。 (SITES-35370)
* 修正了“描述”富文本字段中的焦点管理问题，此前该问题会阻止使用 Tab 键向前移动焦点。 用户在 RTE 中卡住，因为组件依赖非标准键盘命令来移动焦点，这破坏了预期的对话框导航。 此更改强制使用标准键盘进行交互操作，维护了整个对话框中符合逻辑的 Tab 排序。 （SITES-35228）
* 修复了在 Sites 编辑器中创作页面时导致预期行为中断以及组件交互不一致的问题。 作者遇到不可靠的 UI 响应，这干扰了标准编辑任务，降低了工作流效率。 此更新改进了底层编辑器逻辑，并恢复了受影响组件中稳定且可预测的交互行为。 （SITES-35227）
* 一个回归问题导致页面编辑器中的资产选择器失效，并且在特定页面编辑场景下无法加载。 现在，作者在编辑页面过程中选择或浏览资产时，可以正常打开并使用资产选择器。 此更改恢复了对因加载失败而中断的资产选择工作流的稳定访问。 （SITES-35226）
...
* 修复了色板弹出窗口的焦点处理问题。 此对话框现在会将焦点移至清晰的标题，并从该入口点开始屏幕阅读器输出。 NVDA 不再按错误顺序读取完整的对话框内容。 (SITES-25275)
...
* 修复了 320px 宽度情况下的搜索模态布局问题。 模态内容现在可以整齐地重排，避免与树目录重叠。 用户可以查看结果，并在控件不被遮挡的情况下浏览目录。 （SITES-25246）
* 文本间距增加后，搜索模态文本不再被截断。 树目录布局现在保持清晰的分隔，使标签和条目保持清晰可读。 用户现在可以在文本不重叠且不被截断的情况下完成搜索和导航。 （SITES-25245）
...
* AEM 现在为 Teaser 模态信息按钮使用一个有意义的无障碍标签。 屏幕阅读器会读出一个清晰的操作名称，而不是默认图标替代文本字符串。 （SITES-25223）
* 屏幕阅读器现在会在用户按下编辑按钮时读出正确的操作。 NVDA 不再报告“按下了预览按钮”，这种提示在键盘导航过程中曾导致误导性反馈和混淆。 （SITES-25208）
...
* 修复了“用户首选项”模态中在色板导航期间 NVDA 反馈错误的问题。 NVDA 现在会读出获得焦点的色板的标签，从而消除了误导性的颜色输出。 色板集现在支持一致的键盘导航，并让用户清楚感知当前选择。 （SITES-24739）
* 减少了 `Spin` 控件的冗长 NVDA 输出。 移除了与输入标签重复的冗余组标签，因此 NVDA 只公告一次控件名称。 现在，键盘和屏幕阅读器导航功能可提供清晰的单次公告。 （SITES-24725）
* 轮播对话框现在将焦点放在对话框标题上，而不是“项”选项卡上。 “取消”和 Esc 会将焦点恢复到启动此对话框的控件上，这减少了冗长的 NVDA 输出。 (SITES-24716)
...
* 现在，链接打开选择对话框在“关闭”或“取消”后会将键盘焦点恢复到对话框的触发器。 焦点不再跳到“链接”输入框，这可以保持屏幕阅读器上下文稳定，减少额外导航。 (SITES-24707)
* 图像模态对话框现在遵循一个符合逻辑的焦点顺序。 取消后，焦点不再跳过之前的控件，也不会落到页面地标上，退出后焦点会重新回到“配置”按钮上。 (SITES-24693)
...
* 搜索字段现在使用持续显示的屏幕标签，而不是占位符文本。 标签在输入时保持可见，这提高了键盘、屏幕阅读器和语音用户的清晰度。 (SITES-24529)
...
* 图像组件对话框不再将装饰性的帮助图标和全屏图标显示为图像。 屏幕阅读器现在会跳过这些图标，使焦点保持在可操作控件和字段内容上。 (SITES-2940)
* 网站管理员现在可以从文件夹缩略图图标中移除图像角色。 辅助技术会跳过这些装饰性元素，使焦点保持在文件夹名称和操作上。 （SITES-2852）
* 内容树现在会将键盘焦点路由到活跃的树项目或第一个树项目上。 树容器不再用作空的 Tab 停止位，这防止了出现 Shift+Tab 焦点陷阱。 (SITES-1577)

#### 管理员用户界面{#sites-adminui-65-lts-sp2}

Sites 控制台列表视图设置未反映出列表视图中显示的列。 对话框打开后，复选框全部被清除，并且选中的列数不正确。 此修复将对话框状态与活跃的网格列同步，并更新计数器，以匹配实际的列可见性。 (SITES-38576)
...
* 内容片段编辑器现在会本地化“标签”标签。 编辑器还会将收藏集标签本地化，因此 UI 文本符合选定的区域设置。 (SITES-977)


#### [!DNL Content Fragments] - 片段编辑器{#sites-fragments-editor-65-lts-sp2}

* 当功能切换开关在重构后保持禁用状态时，内容片段变体标记消失。 此修复恢复了对变体标记的支持，即使该切换开关保持关闭也是如此。 作者可以在内容片段编辑器中重新添加和查看变体标记。 (SITES-38682) 重要
...
* 对于允许的资产类型，内容片段编辑器现在会显示翻译过的名称。 作者配置内容引用限制时，选取器列表不再混合内部字符串和只用英语的标签。 (SITES-29699)

#### [!DNL Content Fragments] - GraphQL API {#sites-graphql-api-65-lts-sp2}

* 改进了 GraphQL 查询验证处理，以防止因筛选执行错误而导致部署失败。 此错误导致了应用程序启动时出现异常，使得在受影响的环境中部署不成功。 此修复确保了一致的验证行为，并使部署能够顺利进行，而不会受到运行时查询验证中断的影响。 (SITES-34301) 重要
...
* 改进了 GraphQL 查询验证处理，以防止因筛选执行错误而导致部署失败。 此错误导致了应用程序启动时出现异常，使得在受影响的环境中部署不成功。 此修复确保了一致的验证行为，并使部署能够顺利进行，而不会受到运行时查询验证中断的影响。 (SITES-35529)
...
* 当重构操作将此功能与禁用的切换开关绑定时，嵌套内容片段模型停止工作。 此修复恢复了嵌套模型支持，无需更改切换开关。 作者可以在模型编辑器中重新创建并使用嵌套模型。 (SITES-38681) 重要
...
AEM Headless 需要一个专门的发布分支，以避免与主线版本之间的依赖项及捆绑包版本的冲突。 此更新添加了一个 `release/6.5lts` 无头分支，使依赖集和捆绑包版本保持一致。 Jenkins 现在可以顺利构建无头代码库，不再出现版本冲突。 (SITES-36585)

<!-- #### Component console{#sites-component-console-65-lts-sp2} -->

#### 内容 API{#sites-content-api-65-lts-sp2}

功能切换缺陷导致页面管理 API 状态被错误报告。 此更新添加了一个专门的启用标志，将其与现有的切换一起进行评估。 页面管理 API 现在显示为稳定状态。 网站管理 API 保持试验性。 (SITES-39284)
...
* 改进了 Sites 创作行为，解决了在组件交互时页面编辑中断并导致结果不一致的问题。 作者遇到了意外的 UI 响应，这干扰了内容更新，降低了工作流的可靠性。 此更改恢复了稳定的编辑器状态管理，确保了在各种受影响的场景中可预测地执行创作操作。 (SITES-34499)
...
* Sites 时间线在发布提升操作中显示硬编码的英语文本：“Created version ... before promoting launch”（在发布提升之前创建了版本…）。 此更新将硬编码的字符串替换为本地化消息处理机制。 时间线现在显示本地化的文本，使此条目与标准 AEM 本地化行为保持一致。 (SITES-39157)
...
* AEM 发布现在为 6.5 LTS 分支运行专用的 Jenkins 流水线。 该流水线每晚运行构建，并通过电子邮件发送失败警报。 此设置提高了测试覆盖范围，可尽早捕获回归错误。 (SITES-35293)
* AEM 6.5 LTS 现在随附更新后的发布 API 捆绑包，其工件版本已保持一致。 此捆绑包会跟踪主要代码行，同时保持正确的 6.5 LTS 发行版本。 此更新稳定了在 6.5 LTS 技术栈中各处的发布 API 消耗。 (SITES-35292)
* AEM 6.5 LTS 现在包含更新后的 launches-core 捆绑包，其依赖项版本已保持一致。 此更新为“片段 UUID”和“引用 UUID”数据类型添加了 launches-core 处理。 现在，发布处理可在所有发布和内容片段工作流中保持行为一致。 (SITES-35290)
* 改进了 Sites 编辑器，解决了导致正常页面创作工作流中断的行为不一致的问题。 作者遇到意外的组件交互，这干扰了内容更新，降低了编辑可靠性。 此更改恢复了一致的 UI 状态管理，并确保在各种受影响的场景中可预测地执行创作操作。 (SITES-35138)
* 发布编辑现在显示的是本地化的错误文本，而不是硬编码的 `Provided path is not a launch` 字符串。 现在，当“编辑”获得无效的发布路径时，UI 会在所有语言下渲染翻译过的消息。 (SITES-33360)
* AEM 6.5 LTS 现在包含发布 OpenAPI 的侧向移植工作。 此更新将发布 API 捆绑包、内容包和所需的快速启动工件的版本对齐，并为内容片段发布 OpenAPI 场景启用了稳定的 CI 验证。 (SITES-32050)
* 发布 UI 现在会将覆盖的模板标签本地化。 模板覆盖详细信息现在显示翻译过的文本，而不是只有英语的字符串。 (SITES-29525)
* AEM 解决了 **Sites** > **发布** > **编辑**&#x200B;中缺少本地化键的问题。 用户现在会看到翻译过的错误消息，而不是原始的“Unable to update launch source list”字符串。 (SITES-21499)
...
页面编辑器发布操作现在可以处理包含选择符或后缀的 URL。 所发布的请求现在会发送 JCR 页面路径，而不是选择器或后缀 URL 字符串，因此可完成激活，内容可上线。 复制操作现在会在失败时返回一个错误状态，从而防止出现“发布已开始”的假消息。 (NPR-43288)
...
* OSGi 默认配置已设置为 `preview=true`，强制通用编辑器在预览模式下启动。 此更新修正了默认值，并恢复了标准生产条目行为。 除非管理员明确启用预览模式，否则通用编辑器现在会在生产模式下打开。 (SITES-37193)
...
* 用户在自适应表单仪表板中遇到安全漏洞。 具体而言，在 startpointcontrol.js 文件中发现了一个跨站点脚本 (XSS) 问题，它可能会允许执行恶意脚本。 (FORMS-20679)
...
**已在AEM Forms 6.5 LTS Service Pack 2 JEE修补程序（版本6.6.0-0006）中修复以下问题。 要应用这些修补程序，请[下载并安装修补程序](/help/forms/using/adobe-experience-manager-forms-65-lts-hotfixes.md)。**

* 在Apache Shiro更新到版本2.1.0后，JEE上的AEM Forms未能使用Shiro安全管理器的`NoClassDefFoundError`进行引导。 现在，引导已成功完成。 (FORMS-26818)
* JEE上的AEM Forms失败，出现`org.owasp.esapi.reference.JavaLogFactory`的“未找到类”错误。 现在已解析缺少的类。 (FORMS-26819)
* 升级到AEM Forms 6.5 LTS后，删除TaskManager端点。 现在，端点会被保留。 （FORMS-26584和FORMS-26589）
* 在JEE上，由于安全XML生成器，配置管理器MergeEars步骤失败并出现DOCTYPE声明错误(`ALC-LCM-010-200`)。 MergeEars现在已完成。 (FORMS-26569)
* ® WebSphere® Liberty部署中缺少应用程序级别的日志。 应用程序级别的日志记录现已恢复。 (FORMS-25063)
* 在JBoss®上，电子邮件失败，显示“IMAPProvider不是子类型”。 电子邮件现在可用于JBoss®。 (FORMS-24892)
* 在WebSphere® Liberty Profile (WLP)上，电子邮件失败，显示“无法将套接字转换为TLS。” 通过TLS发送的电子邮件现在可用于WLP。 (FORMS-24692)
* 已将Gibson库更新到版本6.0.29665850。 (FORMS-26688)
* 反向移植SAML断言验证改进。 (FORMS-25222)
* 已将Apache Log4j更新到版本2.25.5。 （FORMS-26733和FORMS-26734）
* 此修补程序还包括安全修补程序。
...
* Sling 资源访问安全现已采用 1.1.2 版本。 当多个 ResourceAccessGateHandler 服务注册时，ResourceAccessSecurityImpl 在初始化时不再抛出 ClassCastException。 现在可以可靠完成初始化，避免了在具有多个处理程序的环境中启动失败。 （NPR-42750）
...
* AEM 现在会避免在生成的 `WEB-INF/resources/provisioning/model.txt` 中 `contributor` 组有重复的 ACL 条目。 WAR 输出现在包含一个一致的 ACL 块，从而防止在审阅期间出现令人困惑的权限差异。 (GRANITE-63269)
...
* Sling 更新了管理登录允许列表，以使用包容性术语和新的配置 PID。 此更改与 Sling JCR Base 3.2.0 一致。 (GRANITE-63756)
...
* AEM 6.5 LTS SP2 更新了 Sling、Oak 和 Felix 的基础层捆绑包集。 这些升级增强了核心运行时的稳定性，并统一了整个平台上的依赖项版本。 (GRANITE-61874)

...
* **FORMS-24688：**&#x200B;在Linux上设置JBoss EAP 8.0 (AEM Forms 6.5.1 LTS)时，在Windows上修改的脚本可能包含CRLF行结尾，导致/bin/sh^M：解释器错误或$&#39;\r&#39;：命令未找到错误。 使用dos2unix将受影响的.sh文件转换为Unix (LF)格式(例如，查找。 -type f -name“*.sh” -exec dos2unix {} +)以解决此问题。

* **FORMS-24689：**&#x200B;在Turnkey升级模式下，不会从UI中获取数据库端口的更新，必须在lc_turnkey.xml配置文件中手动修改这些更新。 如果未正确更新，系统可能会继续使用旧端口值。

* **FORMS-24687：**&#x200B;在JBoss EAP 8上的AEM Forms 6.5.1 LTS群集部署中，`domain/configuration/domain_oracle.xml`可能包含重复的`<security>`标记，从而导致无法启动域控制器的XML无效。

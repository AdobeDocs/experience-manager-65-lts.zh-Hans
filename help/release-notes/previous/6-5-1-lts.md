...
>[!IMPORTANT]
>
> **强制热修复**：如要避免在安装服务包 1 时发生离线压缩的 SNFE (SegmentNotFoundException) 问题，请安装[已知问题 – 在线压缩过程中存储库损坏](#repository-corruption-during-online-compaction-after-offline-compaction-granite-65146)中描述的热修复程序。
...
JEE 上的 AEM 6.5 Forms LTS 现已推出。 有关受支持环境的详细信息，请参阅[受支持的平台](/help/forms/using/aem-forms-jee-supported-platforms.md)组合文档。 [AEM Forms 发行版本](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases)页面上提供安装程序链接。

#### AEM Forms 6.5 LTS SP1 中包含的内容

**Java 支持更新**

引入了对较新 Java 版本的支持：

* Oracle Java™ 17/21（64 位）（请参阅 [Java 支持](#java-support)部分中的更多详细信息。

**应用程序服务器支持更新**

* 添加了对 JBoss® EAP 8 的支持。
* 移除了旧版 PicketBox 安全框架。
* 基于 Elytron 的凭据存储库现在支持安全凭据管理。

**配置：凭据存储库（基于 Elytron）**

JBoss® EAP 8 上的 AEM Forms 使用 Elytron 管理安全凭据。 客户必须配置一个基于 Elytron 的凭据存储库，以确保服务器成功启动和安全数据库身份验证。
...
* Jakarta EE 9 引入了命名空间更改，即从 `javax.*` 改为 `jakarta.*`
* 所有&#x200B;**自定义 DSC** 都必须迁移到 `jakarta.*` 命名空间
* AEM Forms 6.5 LTS SP1 仅支持&#x200B;**基于 Jakarta EE 9+ 的应用程序服务器**

有关详细信息，请参阅&#x200B;**从 Javax 迁移到 Jakarta 命名空间**。

#### 从 `javax` 迁移到 `jakarta` 命名空间

从 **AEM Forms 6.5 LTS SP1** 开始，只有实施了 **Jakarta Servlet API 5/6** 的应用程序服务器受到支持。 随着 **Jakarta EE 9 及更高版本**&#x200B;的推出，所有 API 都从 `javax.{}` 命名空间转换为 `jakarta.`。

因此，**所有自定义 DSC 都必须使用 `jakarta` 命名空间**。 使用 `javax.{}` API 构建的自定义组件与受支持的应用程序服务器&#x200B;**不兼容**。

**自定义 DSC 的迁移选项**

您可以使用以下方法之一迁移现有的自定义 DSC：

**选项 1：源代码迁移（推荐）**

* 将所有导入语句从 `javax.{}` 更新为 `jakarta.`
* 将自定义 DSC 项目重新构建并重新编译
* 将更新后的组件重新部署到应用程序服务器

**优势：**

* 确保与 Jakarta EE 9+ 长期兼容
* 最适合用于主动维护的项目

**选项 2：使用 Eclipse 转换器进行二进制迁移**

* 使用 **Eclipse 转换器**&#x200B;工具将已编译的二进制文件 (`.jar`， `.war`) 从 `javax` 转换为 `jakarta`
* 无需更改源代码或重新编译
...
下表显示了从 `javax` 迁移到 `jakarta` 时必需的常见的命名空间更改：
...
在更新自定义 DSC 源代码或验证转换后的二进制文件时，使用这些映射作为参考。
...
>[!NOTE]
>
> （仅 Forms）一次性安装需要 Oracle Java™ 8/11（64 位）。
...
**环境**
适合：安装服务包 1 (SP1) 的 AEM 6.5 LTS（内部部署）客户。 SP1 以 Quickstart JAR 形式交付。

**为什么这很重要**
用于 AEM 6.5 LTS 的 SP1 以 Quickstart JAR 形式提供，而不是需要通过包管理器安装的 ZIP 文件。 内部部署客户可通过替换 Quickstart JAR、解压并重新启动的方式进行升级。 此方法与 Adobe 的就地升级流程保持一致。
...
* **FORMS-24688：**&#x200B;在 Linux 上设置 JBoss EAP 8.0 (AEM Forms 6.5.1 LTS) 时，在 Windows 中创建或编辑的脚本可能包含 CRLF 行末尾，这会导致 /bin/sh^M: bad interpreter 或 $&#39;\r&#39;: command not found 错误。 确保受影响的文件使用 LF 行末尾。 为此可以使用 dos2unix 等工具或任何等效方法。

* **FORMS-24689：**&#x200B;在 Turnkey 模式下，在全新安装或升级时未正确应用数据库端口更新。 在全新安装模式下，用户只能从屏幕上显示的端口中进行选择。 在升级模式下，必须先在 lc_turnkey.xml 中手动更新新数据库端口，然后才能继续升级。

* **FORMS-24687：**&#x200B;在 JBoss EAP 8 上 AEM Forms 6.5 LTS 群集部署中，domain/configuration/domain_oracle.xml、domain_mysql.xml 和 domain_mssql.xml 文件可能包含一个重复的 `<security>` 标记，这导致了无效的 XML，使域控制器无法启动。

### Granite

### 离线压缩后，执行在线压缩时存储库损坏 (GRANITE-65146) {#repository-corruption-during-online-compaction-after-offline-compaction-granite-65146}

如果离线压缩以前在 JCR 存储库上运行，在线压缩时用户遇到了存储库损坏的情况。 在这个场景中可能会出现 `SegmentNotFoundException` (SNFE)。 这可能导致存储库损坏。

要解决此问题，请通过[软件分发](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-65385-1.0.zip)中的包管理器安装热修复程序。 由于热修复程序包含一个低级 `oak-segment-tar` 捆绑包，因此实例会在安装后重新启动。

应用程序时，计划好实例的停机时间。 对于离线压缩，请使用相应的 [Oak-run jar](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar)，也在软件分发中提供。

>[!NOTE]
>
> 对于任何 Oak 运行的操作，请使用 [Oak-run 1.78.1-B006 jar](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar)（上面的链接）。
...
### AEM 6.5 LTS SP1中缺少`com.adobe.granite.apicontroller`包(GRANITE-67639) {#missing-apicontroller-bundle-granite-67639}

AEM 6.5 LTS SP1中缺少`com.adobe.granite.apicontroller`包。 此捆绑包控制如何解析OSGi捆绑包，并可阻止捆绑包解析为其他捆绑包，这对于限制公开的API很有用。

从[软件分发](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-67639-1.0.zip)安装修补程序以使用此功能。
...
* [Experience Manager 6.5 LTS 服务包 1](/help/release-notes/assets/65lts_sp1_bundles.zip) 中包含的 OSGi 捆绑包列表 <!-- UPDATE FOR EACH NEW RELEASE -->
* [Experience Manager 6.5 LTS 服务包 1](/help/release-notes/assets/65lts_sp1_packages.zip) 中包含的内容包列表 <!-- UPDATE FOR EACH NEW RELEASE -->

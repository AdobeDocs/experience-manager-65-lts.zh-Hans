...
>[!IMPORTANT]
>
> **强制性修补程序** — 为避免在安装Service Pack 1时出现脱机压缩的SNFE (SegmentNotFoundException)问题，请安装[已知问题 — 联机压缩期间存储库损坏](#repository-corruption-during-online-compaction-after-offline-compaction-granite-65146)中描述的修补程序。
...
* Oracle Java™ 17/21（64位）(请参阅[Java支持](#java-support)部分中的更多详细信息。
...
>[!NOTE]
>
> （仅限Forms）一次性安装需要Oracle Java™ 8/11（64位）。
...
**环境**
适用于：安装Service Pack 1 (SP1)的AEM 6.5 LTS（内部部署）客户。 SP1 以 Quickstart JAR 形式交付。

**为什么这很重要**
适用于AEM 6.5 LTS的SP1作为快速入门JAR发货，而不是作为要通过包管理器安装的ZIP。 内部部署客户可通过替换 Quickstart JAR、解压并重新启动的方式进行升级。 此方法与 Adobe 的就地升级流程保持一致。
...
* **FORMS-24688：**&#x200B;在Linux上设置JBoss EAP 8.0 (AEM Forms 6.5.1 LTS)时，在Windows中创建或编辑的脚本可能包含CRLF行结尾，导致/bin/sh^M：解释器错误或$&#39;\r&#39;：命令未找到错误。 确保受影响的文件使用LF行尾。 这可以使用dos2unix或任何等效方法等工具实现。

* **FORMS-24689：**&#x200B;在Turnkey模式下，在全新安装或升级期间未正确应用数据库端口更新。 在全新安装模式下，用户只能从屏幕上显示的端口中进行选择。 在升级模式下，必须在继续升级之前在lc_turnkey.xml中手动更新新数据库端口。

* **FORMS-24687：**&#x200B;在JBoss EAP 8上的AEM Forms 6.5 LTS群集部署中，domain/configuration/domain_oracle.xml、domain_mysql.xml和domain_mssql.xml文件可能包含重复的`<security>`标记，从而导致无效XML导致域控制器无法启动。

### Granite

### 离线压缩后在线压缩期间存储库损坏(GRANITE-65146) {#repository-corruption-during-online-compaction-after-offline-compaction-granite-65146}

如果以前在JCR存储库上运行过离线压缩，则用户在在线压缩期间会遇到存储库损坏的问题。 此方案中可能会出现`SegmentNotFoundException` (SNFE)。 这可能导致存储库损坏。

要解决此问题，请通过[Software Distribution](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-65385-1.0.zip)中的包管理器安装修补程序。 由于该修补程序包含一个低级`oak-segment-tar`捆绑包，因此实例会在安装后重新启动。

规划应用实例时的停机时间。 若要离线压缩，请使用相应的[Oak-run jar](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar)，该工具也在Software Distribution上提供。

>[!NOTE]
>
> 对于任何Oak运行的操作，请使用[Oak-run 1.78.1-B006 jar](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar)（上面链接）。

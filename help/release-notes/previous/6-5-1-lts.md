...
* **FORMS-24688：**&#x200B;在Linux上设置JBoss EAP 8.0 (AEM Forms 6.5.1 LTS)时，在Windows上修改的脚本可能包含CRLF行结尾，导致/bin/sh^M：解译器错误或$&#39;\r&#39;：命令未找到错误。 使用dos2unix将受影响的.sh文件转换为Unix (LF)格式(例如，查找。 -type f -name“*.sh” -exec dos2unix {} +)以解决此问题。

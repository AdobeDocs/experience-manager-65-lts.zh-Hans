...
* **FORMS-24688：**&#x200B;在Linux上设置JBoss EAP 8.0时，在Windows上修改的Shell脚本可能包含CRLF行结尾，导致`/bin/sh^M: bad interpreter`或$`\r`：命令未找到错误。 使用dos2unix将受影响的文件转换为LF行结尾以解决此问题。

* **FORMS-24689：**&#x200B;在Turnkey升级模式下，在升级期间可能无法正确应用`lc_turnkey.xml`中的数据库端口，从而导致进程引用旧端口值。

---
title: 使用工作流程
description: 通过AEM Workflows，可自动执行在页面或资源上执行的一系列步骤。 例如，在发布时，编辑者必须在站点管理员激活页面之前审查内容。自动完成此示例的工作流会通知每位参与者何时需要执行其所需工作。
...
通过AEM Workflows，可自动执行在页面或资源上执行的一系列步骤。 例如，在发布时，编辑者必须在站点管理员激活页面之前审查内容。自动完成此示例的工作流会通知每位参与者何时需要执行其必须完成的工作：
